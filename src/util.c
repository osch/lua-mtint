...
lua_Number mtint_current_time_seconds()
{
    lua_Number rslt;
...
bool mtint_membuf_init(MemBuffer* b, size_t initialCapacity, lua_Number growFactor)
{
...
void mtint_membuf_free(MemBuffer* b)
{
    if (b->bufferData) {
        free(b->bufferData);
        b->bufferData     = NULL;
 ...
 */
int mtint_membuf_reserve(MemBuffer* b, size_t additionalLength)
{
...
void mtint_util_quote_lstring(lua_State* L, const char* s, size_t len)
{
    if (s) {
        luaL_Buffer tmp;
        luaL_buffinit(L, &tmp);
...
void mtint_util_quote_string(lua_State* L, const char* s)
{
    mtint_util_quote_lstring(L, s, (s != NULL) ? strlen(s) : 0);
}
