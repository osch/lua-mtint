...
C_FLAGS             = -Wall -pthread -DMTINT_VERSION=scm-`date +"%Y-%m-%d-%H%M%S"`
...
define COMPILE_FLAGS_CMAKE_DEFS
    -D LUAMTINT_COMPILE_FLAGS="`$(PKG_CONFIG) --cflags $(LUA_PKG_CONFIG_NAME)`" \
    -D CMAKE_C_FLAGS="$(C_FLAGS)"
endef

define  LINK_FLAGS_CMAKE_DEFS
    -D LUAMTINT_LINK_FLAGS="`$(PKG_CONFIG) --libs $(LUA_PKG_CONFIG_NAME)`" \
